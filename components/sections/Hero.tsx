...
export default function Hero() {
  ...
  return (
    <section ref={container} className="relative min-h-[819px] flex flex-col justify-center mb-24 px-4 md:px-14 overflow-hidden">
      ...
      <div className="sys-coord absolute left-4 lg:left-8 top-1/2 -translate-y-1/2 -translate-x-full md:translate-x-0 origin-left -rotate-90 text-accent font-mono text-[14px] leading-[160%] opacity-0 tracking-widest hidden lg:block">
      ...
      <div className="flex items-center w-full max-w-7xl mx-auto z-10">
        <div className="w-full z-10 text-left">
          <div className="flex flex-col md:flex-row items-start md:items-center gap-6 mb-4">
            ...
            <h1 className="font-display text-primary text-[32px] sm:text-[48px] md:text-[84px] leading-none tracking-widest whitespace-pre-wrap">
              <span className="hero-title"></span><span className="hero-cursor opacity-0 cursor-blink font-mono">_</span>
            </h1>
          </div>

          <div className="hero-terminal opacity-0 translate-y-4 font-mono text-[14px] leading-[160%] text-accent mb-8 space-y-2 max-w-2xl bg-card border border-accent/30 p-4 relative">
            <div className="absolute top-0 right-0 p-1 text-[10px] bg-accent text-surface">LOG</div>
            <div className="flex flex-col gap-2">
              ...
              <p className="boot-line opacity-0 text-primary">{`> [OK] SYSTEM READY.`}</p>
            </div>
          </div>
        </div>
      </div>

      <div className="status-strip opacity-0 absolute bottom-0 left-0 w-full border-t border-primary/30 py-2 px-14 flex justify-between font-mono text-[14px] text-primary/70">
